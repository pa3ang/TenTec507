/* PatriotAllianceMod   version 1.5
...
#include <Wire.h>  // for I2C on Ten-Tec board
...
void setup() 
{
  ...
  Serial.begin(38400);
  // serial for HC06 bluetooth module used for android display
  Serial1.begin(9600);
...
void loop()    
{
  ...
  if (keyerState == IDLE && millis() - idletimer > 500) {  // no polling and user interaction allowed during cw keying
    ...
    {
      if( !ST_key ) checkWPM();
      Serial1.println( (frequency_tune - IF) * 0.001);
...
void sendmsg(char *str){
  while (*str) {
    ...
    {
      // stop automatic transmission CQ after letter done
      return;
    }
    send(*str++) ;
  }  
...
int rit;
    ...
    if( c != ';' ) { terminal = 1; return; }   // LEAVE 1  command not complete yet but need to switch of terminal
...
int split =0 ;
    ...
    if( lcom == "FA" ){    // set vfo freq 
      ...
      if ( value == 1 || value == 2) { Change_Mode(0); }
      if ( value == 3) {  Change_Mode(1); }
     }
}
...
void Change_Mode(int new_mode)
{
  ...
  frequency_old=0; 
  UpdateFreq(frequency_tune); 
  if ( mode == 1 ) { b = 0x80; Selected_BW = Narrow_BW; }
  if ( mode == 0 ) { b = 0x00; Selected_BW = Wide_BW; }
...
void  Selection()
{
    ...
    {   
       ...
       while( digitalRead(Select_Button) == HIGH ){ 
         ...
         // function button is pressed longer then 0.5 seconds
         if ( (millis() - long_time) > 500 ) { 
           if  ( Step_Multi_Function_Button1 == 2 && mode == 1 ) {
             // send cq
             s = s | 0x04; 
             Select_Multi_BW_Ored();
             sendmsg(CQ);
